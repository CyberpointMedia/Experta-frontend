class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  // Onboarding images
  static String imgRectangle103 = '$imagePath/img_rectangle_103.png';

  static String imgRectangle106 = '$imagePath/img_rectangle_106.png';

  static String imgRectangle101 = '$imagePath/img_rectangle_101.png';

  static String imgRectangle108 = '$imagePath/img_rectangle_108.png';

  static String imgRectangle102 = '$imagePath/img_rectangle_102.png';

  static String imgRectangle107 = '$imagePath/img_rectangle_107.png';

  // static String imgRectangle109 = '$imagePath/img_rectangle_109.png';

  static String imgRectangle105 = '$imagePath/img_rectangle_105.png';

  static String imgRectangle104 = '$imagePath/img_rectangle_104.png';

  static String imgSmartphone = '$imagePath/img_smartphone.svg';

  static String imgGoogle = '$imagePath/img_google.svg';

  static String imgFacebook = '$imagePath/img_facebook.svg';

  static String imgSocialIcon = '$imagePath/img_social_icon.svg';

  // splash images
  static String imgVector = '$imagePath/splashlogo.svg';

  // SignIn images
  // static String imgTelevision = '$imagePath/img_television.svg';

  static String imgArrowDown = '$imagePath/img_arrow_down.svg';
  // static String imgRectangle2232x164 = '$imagePath/img_rectangle_2_232x164.png';

  // static String imgRectangle2232x163 = '$imagePath/img_rectangle_2_232x163.png';

  // static String imgRectangle3232x163 = '$imagePath/img_rectangle_3_232x163.png';

  // static String imgRectangle3232x164 = '$imagePath/img_rectangle_3_232x164.png';

  // static String imgRectangle32 = '$imagePath/img_rectangle_3_2.png';

  // static String imgRectangle3220x164 = '$imagePath/img_rectangle_3_220x164.png';

  // static String imgRectangle33 = '$imagePath/img_rectangle_3_3.png';

  // User Profile / Posts images
  // static String imgRectangle25244 = '$imagePath/img_rectangle_25244.png';

  // static String imgRectangle25246 = '$imagePath/img_rectangle_25246.png';

  // static String imgRectangle25245 = '$imagePath/img_rectangle_25245.png';

  // static String imgRectangle25248 = '$imagePath/img_rectangle_25248.png';

  // static String imgRectangle25247 = '$imagePath/img_rectangle_25247.png';

  // static String imgRectangle25249 = '$imagePath/img_rectangle_25249.png';
//
  // static String imgFaceBlowingAKiss = '$imagePath/img_face_blowing_a_kiss.png';

  // static String imgGrinningFaceWith = '$imagePath/img_grinning_face_with.png';

  // User Profile images
  // static String imgPhone = '$imagePath/img_phone.svg';
  //  static String imgArrowdownBlueGray300 =
  //     '$imagePath/img_arrowdown_blue_gray_300.svg';

  // static String imgClose = '$imagePath/img_close.svg';

  // static String imgCloseOnprimarycontainer =
  //     '$imagePath/img_close_onprimarycontainer.svg';

  // Video Call One images
  // static String imgVideoCallOne = '$imagePath/img_video_call_one.png';

  // static String imgMaskGroup122x91 = '$imagePath/img_mask_group_122x91.png';

  // static String imgFlipCameraIos = '$imagePath/img_flip_camera_ios.svg';

  static String imgMoreHorizontal = '$imagePath/img_more_horizontal.svg';

  // Audio Call One images
  // static String imgGroup1038 = '$imagePath/img_group_1038.png';

  // Audio Call images
  // static String imgRectangle2288x355 = '$imagePath/img_rectangle_2_288x355.png';

  // static String imgImage33 = '$imagePath/img_image_33.png';

  // static String imgRectangle2120x120 = '$imagePath/img_rectangle_2_120x120.png';

  // static String imgRectangle3120x120 = '$imagePath/img_rectangle_3_120x120.png';

  // static String imgImage33120x120 = '$imagePath/img_image_33_120x120.png';

  // static String imgRectangle2126x375 = '$imagePath/img_rectangle_2_126x375.png';

  // static String imgImage33126x374 = '$imagePath/img_image_33_126x374.png';

  static String imgImage34 = '$imagePath/img_image_34.png';

  static String imgPlusCircle = '$imagePath/img_plus_circle.svg';

  // static String imgGear = '$imagePath/img_gear.svg';

  // static String imgRectangle280x80 = '$imagePath/img_rectangle_2_80x80.png';

  // static String imgRectangle380x80 = '$imagePath/img_rectangle_3_80x80.png';

  static String imgImage3380x80 = '$imagePath/img_image_33_80x80.png';

  // New Post / Choose post image images
  //  static String imgArrowRightOnprimarycontainer =
  //      '$imagePath/img_arrow_right_onprimarycontainer.svg';

  // static String imgArrowdownOnprimarycontainer =
  //     '$imagePath/img_arrowdown_onprimarycontainer.svg';

  // static String imgCheckbox = '$imagePath/img_checkbox.svg';

  // static String imgSearchOnprimarycontainer =
  //     '$imagePath/img_search_onprimarycontainer.svg';

  // static String imgRectangle2524593x93 =
  //     '$imagePath/img_rectangle_25245_93x93.png';

  // static String imgRectangle2524693x93 =
  //     '$imagePath/img_rectangle_25246_93x93.png';

  // static String imgRectangle2524793x93 =
  //     '$imagePath/img_rectangle_25247_93x93.png';

  // static String imgRectangle2524893x93 =
  //     '$imagePath/img_rectangle_25248_93x93.png';

  // static String imgRectangle2524993x93 =
  //     '$imagePath/img_rectangle_25249_93x93.png';

  // static String imgFaceBlowingAKiss93x93 =
  //     '$imagePath/img_face_blowing_a_kiss_93x93.png';

  // static String imgGrinningFaceWith93x93 =
  //     '$imagePath/img_grinning_face_with_93x93.png';

  // static String imgGrinningFaceWith1 =
  //     '$imagePath/img_grinning_face_with_1.png';

  // static String imgBeamingFaceWithsmiling =
  //     '$imagePath/img_beaming_face_withsmiling.png';

  // static String imgFrowningFaceWith = '$imagePath/img_frowning_face_with.png';

  // static String imgRectangle248x48 = '$imagePath/img_rectangle_2_48x48.png';

  // static String imgImage3348x48 = '$imagePath/img_image_33_48x48.png';

  static String imgArrowRight = '$imagePath/img_arrow_right.svg';

  // static String imgShield = '$imagePath/img_shield.svg';

  // static String imgGroup76136 = '$imagePath/img_group_76136.svg';

  // static String imgNightMode = '$imagePath/img_night_mode.svg';

  // static String imgStar = '$imagePath/img_star.svg';

  // static String imgInfo = '$imagePath/img_info.svg';

  // static String imgHeadphones = '$imagePath/img_headphones.svg';

  // static String imgPower = '$imagePath/img_power.svg';

  // static String imgFrame31 = '$imagePath/img_frame_31.png';

  // static String imgIconBlueGray30060x60 =
  //     '$imagePath/img_icon_blue_gray_300_60x60.svg';

  // static String imgCheckBlueGray300 = '$imagePath/img_check_blue_gray_300.svg';

  // static String imgLock = '$imagePath/img_lock.svg';

  static String imgInbox2 = '$imagePath/img_inbox_2.svg';

  // static String imgPlay38x38 = '$imagePath/img_play_38x38.png';

  static String imgMoreHorizontalBlueGray300 =
      '$imagePath/img_more_horizontal_blue_gray_300.svg';

  // static String imgRectangle238x38 = '$imagePath/img_rectangle_2_38x38.png';

  // static String imgImage36 = '$imagePath/img_image_36.png';

  // static String imgRectangle23250x311 =
  //     '$imagePath/img_rectangle_23_250x311.png';

  // static String imgArrowUp = '$imagePath/img_arrow_up.svg';

  // static String imgArrowRightGray90016x16 =
  //     '$imagePath/img_arrow_right_gray_900_16x16.svg';

  // static String imgInbox = '$imagePath/img_inbox.svg';

  // static String imgArrowLeftPrimarycontainer =
  //     '$imagePath/img_arrow_left_primarycontainer.svg';

  // static String imgArrowDownGreen400 =
  //     '$imagePath/img_arrow_down_green_400.svg';

  // static String imgUser = '$imagePath/img_user.svg';

  // static String imgLogIn = '$imagePath/img_log_in.svg';

  // static String imgSettingsGray900 = '$imagePath/img_settings_gray_900.svg';

  // static String imgClockErrorcontainer =
  //     '$imagePath/img_clock_errorcontainer.svg';

  // Settings / Notifications images
  // static String imgGroup273 = '$imagePath/img_group_273.png';

  // static String imgGiftBox1 = '$imagePath/img_gift_box_1.svg';

  // static String imgInfoBlueGray30024x24 =
  //     '$imagePath/img_info_blue_gray_300_24x24.svg';

  // static String imgWhatsapp1 = '$imagePath/img_whatsapp_1.svg';

  // static String imgSearchGray90056x56 =
  //     '$imagePath/img_search_gray_900_56x56.svg';

  // static String imgFrame1171276693 = '$imagePath/img_frame_1171276693.svg';

  // static String imgFrame1171276692 = '$imagePath/img_frame_1171276692.svg';

  // static String imgArrowLeftBlueGray300 =
  //     '$imagePath/img_arrow_left_blue_gray_300.svg';

  // static String imgWomanWithHeadsetVideoCall =
  //     '$imagePath/img_woman_with_headset_video_call.png';

  // static String imgExclude = '$imagePath/img_exclude.svg';

  // static String imgGroup1000004498 = '$imagePath/img_group_1000004498.svg';

  // static String imgWomanWithHeadsetVideoCall94x164 =
  //     '$imagePath/img_woman_with_headset_video_call_94x164.png';

  static String imgWomanWithHeadsetVideoCall1 =
      '$imagePath/img_woman_with_headset_video_call_1.png';

  // static String imgWomanWithHeadsetVideoCall2 =
  //     '$imagePath/img_woman_with_headset_video_call_2.png';

  // static String imgRectangle276x76 = '$imagePath/img_rectangle_2_76x76.png';

  // static String imgImage3376x76 = '$imagePath/img_image_33_76x76.png';

  // Common images
  static String imgIcon = '$imagePath/img_icon.svg';

  // static String imgEyeoff = '$imagePath/img_eyeoff.svg';

  // static String imgIconOnprimarycontainer =
  //     '$imagePath/img_icon_onprimarycontainer.svg';

  static String imgArrowLeftOnerrorcontainer =
      '$imagePath/img_arrow_left_onerrorcontainer.svg';

  static String imgCalendar = '$imagePath/settings/calendar.svg';

  static String imgedit = '$imagePath/settings/Frame.svg';
  static String img = '$imagePath/settings/Frame 1171276710.svg';

  // static String imgIconBlueGray300 = '$imagePath/img_icon_blue_gray_300.svg';

  // static String imgCamera = '$imagePath/img_camera.svg';

  // static String imgPlus = '$imagePath/img_plus.svg';

  static String imgSearchBlueGray300 =
      '$imagePath/img_search_blue_gray_300.svg';

  // static String imgCheckGray900 = '$imagePath/img_check_gray_900.svg';

  static String imgBell02 = '$imagePath/img_bell_02.svg';

  // static String imgThumbsUp = '$imagePath/img_thumbs_up.svg';

  // static String imgRunningShoe = '$imagePath/img_running_shoe.svg';

  // static String imgTheater3 = '$imagePath/img_theater_3.svg';

  // static String imgGuitar = '$imagePath/img_guitar.svg';

  // static String imgMovie = '$imagePath/img_movie.svg';

  // static String imgMenu = '$imagePath/img_menu.svg';

  // static String imgChefHat3 = '$imagePath/img_chef_hat_3.svg';

  // static String imgGraduateHat = '$imagePath/img_graduate_hat.svg';

  // static String imgMagicCards = '$imagePath/img_magic_cards.svg';

  // static String imgAmericanFootball = '$imagePath/img_american_football.svg';

  // static String imgRectangle2 = '$imagePath/img_rectangle_2.png';

  // static String imgSignal = '$imagePath/img_signal.svg';

  static String imgLayer1 = '$imagePath/img_layer_1.svg';

  // static String imgRectangle3 = '$imagePath/img_rectangle_3.png';

  // static String imgRectangle3220x156 = '$imagePath/img_rectangle_3_220x156.png';

  // static String imgRectangle2220x156 = '$imagePath/img_rectangle_2_220x156.png';

  // static String imgRectangle21 = '$imagePath/img_rectangle_2_1.png';

  static String imgRectangle22 = '$imagePath/img_rectangle_2_2.png';

  static String imgNavHome = '$imagePath/img_nav_home.svg';

  static String imgNavMessage = '$imagePath/img_nav_message.svg';

  static String imgNavFeeds = '$imagePath/img_nav_feeds.svg';

  static String imgNavProfile = '$imagePath/img_nav_profile.svg';

  static String imgRectangle258x58 = '$imagePath/img_rectangle_2_58x58.png';

  static String imgVerified = '$imagePath/img_verified.svg';

  static String imgVideo = '$imagePath/img_video.svg';

  static String imgRectangle358x58 = '$imagePath/img_rectangle_3_58x58.png';

  // static String imgRectangle31 = '$imagePath/img_rectangle_3_1.png';

  static String imgRectangle23 = '$imagePath/img_rectangle_2_3.png';

  // static String imgRectangle24 = '$imagePath/img_rectangle_2_4.png';

  // static String imgRectangle25 = '$imagePath/img_rectangle_2_5.png';

  static String imgEllipse134 = '$imagePath/img_ellipse_134.png';

  // static String imgSearchGray900 = '$imagePath/img_search_gray_900.svg';

  static String imgNavHomeErrorcontainer =
      '$imagePath/img_nav_home_errorcontainer.svg';

  // static String imgRectangle77 = '$imagePath/img_rectangle_77.png';

  static String imgVideoPrimarycontainer =
      '$imagePath/img_video_primarycontainer.svg';

  // static String imgCalendarPrimary = '$imagePath/img_calendar_primary.svg';

  static String imgCheckmark = '$imagePath/img_checkmark.svg';

  // static String imgClock = '$imagePath/img_clock.svg';

  // static String imgPlay = '$imagePath/img_play.png';

  // static String imgSettings = '$imagePath/img_settings.svg';

  // static String imgArrowLeft = '$imagePath/img_arrow_left.svg';

  // static String imgNavSearch = '$imagePath/img_nav_search.svg';

  // static String imgNavMessageGray900 =
  //     '$imagePath/img_nav_message_gray_900.svg';

  // static String imgNavProfileErrorcontainer =
  //     '$imagePath/img_nav_profile_errorcontainer.svg';

  static String imgPhoneGray900 = '$imagePath/img_phone_gray_900.svg';

  static String imgSmile = '$imagePath/img_smile.svg';

  static String imgIconSolidPaperAirplane =
      '$imagePath/img_icon_solid_paper_airplane.svg';

  static String imgDoubleCheck = '$imagePath/img_double_check.svg';

  // static String imgThumbsUpGreen400 = '$imagePath/img_thumbs_up_green_400.svg';

  // static String imgArrowLeftOnprimarycontainer =
  //     '$imagePath/img_arrow_left_onprimarycontainer.svg';

  // static String imgVolumeUpFill0 = '$imagePath/img_volume_up_fill0.svg';

  // static String imgMicOffFill0W = '$imagePath/img_mic_off_fill0_w.svg';

  // static String imgEndCall = '$imagePath/img_end_call.svg';

  // static String imgMaskGroup60x60 = '$imagePath/img_mask_group_60x60.png';

  // // static String imgVideoOnprimarycontainer =
  //     '$imagePath/img_video_onprimarycontainer.svg';

  // static String imgNavProfileGray900 =
  //     '$imagePath/img_nav_profile_gray_900.svg';

  // static String imgRectangle25244375x375 =
  //     '$imagePath/img_rectangle_25244_375x375.png';

  // static String imgAccountCircle = '$imagePath/img_account_circle.svg';

  static String imgArrowRightGray900 =
      '$imagePath/img_arrow_right_gray_900.svg';

  // static String imgCalendarDeepPurple300 =
  //     '$imagePath/img_calendar_deep_purple_300.svg';

  // static String imgCreditCard = '$imagePath/img_credit_card.svg';

  // static String imgVideoYellow900 = '$imagePath/img_video_yellow_900.svg';

  // static String imgVerifiedUser2 = '$imagePath/img_verified_user_2.svg';

  static String imgCalendarGray900 = '$imagePath/img_calendar_gray_900.svg';

  // static String imgPlayGreen500 = '$imagePath/img_play_green_500.png';

  // static String imgNavFeedsGray900 = '$imagePath/img_nav_feeds_gray_900.svg';

  static String imgInfoBlueGray300 = '$imagePath/settings/inform.svg';

  // static String imgFlash2 = '$imagePath/img_flash_2.svg';

  // static String imgShieldGray900 = '$imagePath/img_shield_gray_900.svg';

  static String noto = "$imagePath/settings/bell.svg";

  static String imageNotFound = 'assets/images/image_not_found.png';

  // ACCOUNT STTING
  static String user = "$imagePath/settings/User.svg";
  static String birthday = "$imagePath/settings/Cake.svg";
  static String gender = "$imagePath/settings/Gender.svg";
  static String email = "$imagePath/settings/Email.svg";
  static String phone = "$imagePath/settings/mobile.svg";
  static String popup = "$imagePath/settings/popup.svg";

  // PAYMENT
  static String wallet = "$imagePath/wallet.svg";
  static String withdraw = "$imagePath/settings/Withdraw.svg";
  static String transaction = "$imagePath/settings/Transaction.svg";
  static String uservarification = "$imagePath/settings/User Verify.svg";
  static String reffer = "$imagePath/Star.svg";
  static String help = "$imagePath/settings/24-hours.svg";
  static String bank = "$imagePath/settings/Bank.svg";
  static String card = "$imagePath/settings/credit-card.svg";
  static String pancard = "$imagePath/settings/credit-card.svg";
  static String block = "assets/images/bookings/block.sv";
  static String delete = "$imagePath/settings/Delete.svg";
  static String upi = "$imagePath/settings/UPI.svg";
  static String hr = "$imagePath/settings/24-hours.svg";

  //gender
  static String male = "$imagePath/settings/male.svg";
  static String maleunselected = "$imagePath/settings/male_unselected.svg";
  static String female = "$imagePath/settings/female.svg";
  static String femele = "$imagePath/settings/femle_unselected.svg";

  static String condition = "$imagePath/settings/terms.svg";
  static String cokie = "$imagePath/settings/Cookie.svg";
  static String term = "$imagePath/settings/condition.svg";

  static String cross = '$imagePath/settings/cross.svg';
  static String plus = '$imagePath/settings/plus.svg';
  static String info = "assets/images/settings/Information.svg";
  static String brief = "assets/images/settings/Brief.svg";
  static String vector = "assets/images/settings/Vector.svg";
  static String union = "assets/images/settings/Union.svg";
  static String avail = "assets/images/settings/available.svg";
  static String price = "assets/images/settings/price.svg";
  static String clock = "assets/images/settings/clock.svg";
  static String uploadcloud = '$imagePath/settings/uploadcloud.svg';

  // albums

  static String recents = "$imagePath/settings/history.png";
  static String videos = "$imagePath/settings/videos.png";
  static String camera = "$imagePath/settings/camera.svg";
  static String multiSelect = "$imagePath/settings/multiselect.svg";

  // camera.svg

  // dashboard

  static String dashboard = "$imagePath/img_group_9.svg";
  static String msg = "$imagePath/settings/msg.svg";
  static String complete = "$imagePath/settings/complete.svg";
  static String singletick = "$imagePath/settings/singletick.svg";

  // sucess

  static String success = "$imagePath/settings/checkSucess.svg";
  static String message = "$imagePath/settings/message.svg";

  // Transactions

  static String topup = "assets/images/wallet/walletTransactions.svg";
  static String deposit = "assets/images/wallet/deposit.svg";
  static String transact = "assets/images/wallet/transact.svg";
  static String receive = "assets/images/wallet/receive.svg";
  static String excel = "assets/images/wallet/Excel.svg";
  static String filtercross = "assets/images/wallet/filtercross.svg";
  static String calender = "assets/images/wallet/calender.svg";
  static String videocam = "assets/images/wallet/videocam.svg";
  static String call = "assets/images/wallet/call.svg";
  static String rate = "assets/images/wallet/verify.svg";
  static String i = "assets/images/wallet/i.svg";
  static String whatsapp = "assets/images/wallet/whatsapp.svg";
  static String ai = "assets/images/wallet/ai.svg";
  static String revert = "assets/images/wallet/revert.svg";

  //  Pan card

  static String pan = "assets/images/bookings/pan.svg";
  static String panset = "assets/images/bookings/panset.svg";
  static String pdf = "assets/images/bookings/pdf.svg";
  static String gst = "assets/call/gst.svg";
  static String imgNoChat = "assets/call/imgnochat.svg";
  static String feeds = "assets/call/feeds.svg";
  static String nointernet = "assets/images/social/nointernet.svg";
}
